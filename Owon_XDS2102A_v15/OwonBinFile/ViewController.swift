...
class ViewController: NSViewController, NSWindowDelegate {
    ...
    private func channelInfoText(prefix: String, channel: Channel?, data: [UInt8]) -> String {
        ...
        return "\(prefix): samples=\(samples), rate=\(channel.Sample_Rate), Vscale=\(channel.Vscale), " +
            "Reference_Zero=\(channel.Reference_Zero), Probe_Magnification=\(channel.Probe_Magnification), Freq=\(channel.Freq)"
    }
    ...
    @IBAction func openDocument(_ sender: NSMenuItem) {
        ...
        do {
            ...
            for (index, channel) in owonFile.waveform.channel.enumerated() {
                ...
                print("\(channel.Index): samples=\(channel.Data_Length), bytes=\(actualBytes), rate=\(channel.Sample_Rate), " +
                    "Vscale=\(channel.Vscale), Reference_Zero=\(channel.Reference_Zero), " +
                    "Probe_Magnification=\(channel.Probe_Magnification), Freq=\(channel.Freq)")
            }
            ...
            let ch1Description = channelDescriptions.indices.contains(0) ? channelDescriptions[0] : nil
            let ch2Description = channelDescriptions.indices.contains(1) ? channelDescriptions[1] : nil

            channelViewOutlet.addChannels(
                channel1.count, channel1,
                channel2.count, channel2,
                ch1ReferenceZero: ch1Description?.Reference_Zero,
                ch2ReferenceZero: ch2Description?.Reference_Zero
            )

            ch1InfoLabel.stringValue = channelInfoText(
                prefix: "CH1",
                channel: ch1Description,
                data: channel1
            )
            ch2InfoLabel.stringValue = channelInfoText(
                prefix: "CH2",
                channel: ch2Description,
                data: channel2
            )
